...
import { Home, Heart, MapIcon, User, Plus, Clapperboard } from "lucide-react";
    ...
    <>
      <nav className="fixed bottom-0 left-0 right-0 z-40 glass border-t border-warm-100/50 md:hidden">
        <div className="page-shell flex items-center justify-around py-2.5 pb-[max(0.625rem,env(safe-area-inset-bottom))]">
          ...
          <button type="button" onClick={() => setChooserOpen(true)} className={`${base} text-warm-400`}>
          ...
          <NavLink to={user ? "/saved" : "/login"} className={cls}>
            <Heart size={20} />
            <span>Saved</span>
          </NavLink>
